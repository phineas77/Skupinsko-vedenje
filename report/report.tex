...
\significancestatement{A herding algorithm that uses fuzzy logic}{Summary of Findings: Recapitulate key findings and their significance.}{fuzzy logic | herding algorithm  | genetic algorithm}
...
% CE BO V NASLEDNJI ITERACIJI PREDOLGO:
% TA DEL BI BILO BOLJE PRESTAVIT V METHODS ALI VSAJ SKRAJSAT, BI REKEL DA JE MALO TOO IN DEPTH ZA INTRODUCTION ZADNJA POLOVICA ODSTAVKA ... done
...
% To enhance the algorithm our predecessors also introduced two improvements.
% As an improvement to the algorithm, our predecessors also introduced two new forces.The first prevents the sheepdog from separating an already collected group, and the second addresses situations where the shepherd gets stuck between two nearby groups of sheep.
